...
def visualize_pair(src_before, src_after, tokenizer):
    ...
    for d in diffs:
        if d.startswith("- "):
            print(
                colored(tokenizer.decode(int(d[2:])), "red", attrs=["reverse"]), end=""
            )
        elif d.startswith("+ "):
            print(
                colored(tokenizer.decode(int(d[2:])), "green", attrs=["reverse"]),
                end="",
            )
        elif d.startswith("  "):
    ...
    if "-lora" in model_name:
        ...
        if "checkpoint-epoch" in model_name:
            ...
            fine_tuned_model_dir = os.path.join(
                args.model_dir, model_name, "checkpoint-last"
            )
        assert os.path.exists(fine_tuned_model_dir)

        tokenizer = AutoTokenizer.from_pretrained(fine_tuned_model_dir)
        model = AutoModelForCausalLM.from_pretrained(
            pretrained_model_dir, device_map="auto", trust_remote_code=True
        )
        ...
        pretrained_model_dir = os.path.join(
            args.model_dir, pretrained_name, "checkpoint-last"
        )
        ...
        model = model_class.from_pretrained(
            pretrained_model_dir, device_map="auto", vocab_size=len(tokenizer)
        )
        ...
        if model_name in PRETRAINED_MODELS or model_name == "deepseek":
            ...
            model = Qwen2ForCausalLM.from_pretrained(model_dir, device_map="auto")
        # model.resize_token_embeddings(len(tokenizer))
...
def try_parse(code, info):
    ...
    if lang == "py":
        ...
        with tempfile.NamedTemporaryFile(
            ...
            if lang == "java":
                temp_file.write(
                    code.replace("MyTestClass", os.path.basename(temp_file_name)[:-5])
                )
...
def inspect_cwe_dist(dataset):
    ...
    print(
        tabulate(
            data,
            headers=["Vulnerability"] + list(langs),
            stralign="right",
            tablefmt="orgtbl",
        )
    )
