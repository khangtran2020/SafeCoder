...
def get_logits_from_lm(lm, inputs, control_ids):
    ...
    return shift_logits.squeeze(0), torch.gather(shift_probs, 2, shift_labels).squeeze(
        -1
    ).squeeze(0)
...
class Trainer:
        ...
        for sample_type in sample_types:
            if sample_type == FUNC:
                loss = token_weighted_loss(
                    "ce", shift_logits, shift_inputs, shift_weights
                )
                loss_dict["func"].append(loss.item())
                loss_total += loss
            elif sample_type == GOOD:
                loss = self.args.loss_weight * token_weighted_loss(
                    "ce", shift_logits, shift_inputs, shift_weights
                )
                ...
                loss = self.args.loss_weight * token_weighted_loss(
                    "ul", shift_logits, shift_inputs, shift_weights
                )
            ...
            if (
                sample_type == GOOD or sample_type == BAD
            ) and self.args.kl_loss_weight > 0:
                with torch.no_grad():
                    ref_outputs = self.ref_model(inputs)
                shift_ref_log_probs = F.log_softmax(
    ...
    def sven_step(self, batch):
        ...
        incorrect_logits, incorrect_label_probs = get_logits_from_lm(
            self.model, inputs, incorrect_control_ids
        )

        contrastive_probs = torch.stack(
            (correct_label_probs, incorrect_label_probs), dim=1
        )
        contrastive_probs = F.normalize(contrastive_probs, p=1, dim=-1)
        contrastive_log_probs = torch.log(contrastive_probs)
        contrastive_labels = torch.zeros(shift_inputs.shape, dtype=torch.int64).to(
            self.model.device
        )
        contrastive_loss = token_weighted_loss(
            "nll", contrastive_log_probs, contrastive_labels, shift_weights
        )
        contrastive_loss *= 4
        ...
        kl_loss = token_weighted_loss(
            "kl", correct_log_probs, ref_log_probs, 1 - shift_weights
        )
        incorrect_log_probs = F.log_softmax(incorrect_logits, dim=-1)
        kl_loss += token_weighted_loss(
            "kl", incorrect_log_probs, ref_log_probs, 1 - shift_weights
        )
    ...
    def do_eval(self):
        ...
        for batch in val_dataloader:
            loss, loss_dict = (
                self.sven_step(batch) if self.args.sven else self.step(batch)
            )
                ...
                set(
                    [
                        ...
                        for name in re.findall(
                            r"\((\w+)\): Linear", str(self.model.modules)
                        )
                    ]
                )
            ),
    ...
    def run(self):
        ...
        self.args.logger.info(
            "  Gradient Accumulation steps = %d", self.args.grad_acc_steps
        )
        ...
        for idx in range(self.args.num_train_epochs):
            ...
            if self.args.save_epochs > 0 and (idx + 1) % self.args.save_epochs == 0:
                ...
                output_dir = os.path.join(
                    self.args.output_dir, f"checkpoint-epoch-{idx+1}"
                )
