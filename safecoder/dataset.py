...
class Upsampler:
    ...
    def upsample(self, safe_dataset, safe_dataset_json, reference_abs):
        if self.sampling_size <= 0:
            ...
            if self.sampling_method == "minority":
                return self._upsample_minority(
                    safe_dataset, safe_dataset_json, reference_abs
                )
            else:
                return self._upsample_all_prob(
                    safe_dataset, safe_dataset_json, reference_abs
                )
    ...
    def _upsample_all_prob(self, safe_dataset, safe_dataset_json, reference_abs):
        ...
        return [safe_dataset[idx] for idx in all_indexes], [
            safe_dataset_json[idx] for idx in all_indexes
        ]
    ...
    def _upsample_minority(self, safe_dataset, safe_dataset_json, reference_abs):
        ...
        for lang, cwe_map in index_map.items():
            for cwe, indexes in cwe_map.items():
                ...
                upsamples.extend(
                    list(np.random.choice(indexes, sample_len, replace=True))
                )

        return safe_dataset + [
            safe_dataset[idx] for idx in upsamples
        ], safe_dataset_json + [safe_dataset_json[idx] for idx in upsamples]
...
class CodeDataset(Dataset):
    def __init__(self, args, tokenizer, mode, num_sample=-1):
        self.args = args
        ...
        if self.mode == "train" and len(safe_dataset) > 0:
            self.args.logger.info(
                "number of sec samples before upsampling: %d", len(safe_dataset)
            )
            safe_dataset, safe_dataset_json = self.upsampler.upsample(
                safe_dataset, safe_dataset_json, len(func_dataset)
            )
            self.args.logger.info(
                "number of sec samples after upsampling: %d", len(safe_dataset)
            )
        # inspect_cwe_dist(safe_dataset_json)
        # print(len(safe_dataset))
    ...
    def load_safe_datasets(self, dnames):
        ...
        if len(dnames) == 0:
            ...
            for dname in dnames:
                with open(
                    os.path.join(self.args.data_dir, self.mode, f"{dname}.jsonl")
                ) as f:
    ...
    def get_safe_samples(self, j):
        ...
        if self.args.sven:
            tokens_complete_good, weights_complete_good = (
                prepd_j["tokens_after"],
                prepd_j["weights_after"],
            )
            tokens_complete_bad, weights_complete_bad = (
                prepd_j["tokens_before"],
                prepd_j["weights_before"],
            )
        else:
            instruction = PROMPT_NO_INPUT.format(instruction=j["description"])
            instruction_tokenized = self.tokenizer.encode_plus(instruction).data[
                "input_ids"
            ]
        ...
        if (not self.args.exclude_neg) and self.check_sample_valid(
            tokens_complete_bad, weights_complete_bad
        ):
    ...
    def _preprocess_safe(self, j):
        ...
        diffs = difflib.ndiff(
            tokens_before_str, tokens_after_str, linejunk=None, charjunk=None
        )
        if self.args.no_weights:
            weights_before, weights_after = [1] * len(tokens_before), [1] * len(
                tokens_after
            )
        else:
            weights_before, weights_after = [0] * len(tokens_before), [0] * len(
                tokens_after
            )
        ...
        tokens_before_trimmed, weights_before_trimmed = self.trim_safe_sample(
            tokens_before, weights_before
        )
        tokens_after_trimmed, weights_after_trimmed = self.trim_safe_sample(
            tokens_after, weights_after
        )
    ...
    def load_func_dataset(self, dataset_name):
        dataset = []
        with open(
            os.path.join(self.args.data_dir, self.mode, f"{dataset_name}.jsonl")
        ) as f:
            lines = f.readlines()
        if self.num_sample > 0:
            lines = lines[: self.num_sample]
        for line in lines:
            j = json.loads(line)
